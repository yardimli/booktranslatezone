...
<div id="clone-modal" class="hidden fixed inset-0 z-50 bg-background/80 backdrop-blur-sm">
	<div class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] gap-4 border bg-card p-6 shadow-lg duration-200 rounded-lg">
		...
		<form id="clone-form" class="space-y-4 pt-2">
			...
			<input type="hidden" id="clone-original-file" name="original_file">
			<div>
				<label for="clone-new-name" class="text-sm font-medium leading-none">File Name (will be slugified)</label>
		...
		<div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-4">
			...
			<button id="save-clone-btn" type="button" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors h-10 px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90">Save File</button>
		</div>
	</div>
</div>
