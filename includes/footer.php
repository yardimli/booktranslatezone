...
<div id="export-modal" class="hidden fixed inset-0 z-50 bg-background/80 backdrop-blur-sm">
	<div
		class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-card p-6 shadow-lg duration-200 rounded-lg">
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
		...
		// --- CLONE & EDIT MODAL (REVISED LOGIC) ---
		...
		function openCloneModal(type) {
			...
			cloneModalTitle.textContent = isEditMode ?
				(isPrompt ? 'Edit Custom Prompt' : 'Edit Custom Example') :
				(isPrompt ? 'Clone & Edit Prompt' : 'Clone & Edit Examples');
