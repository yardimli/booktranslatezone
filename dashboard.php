						...
						<tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
							...
							<td class="p-4 align-middle">
								<div class="flex items-center gap-2">
									...
									<!-- Modified: Add Log button -->
									<?php
										$log_file_path = get_log_path($_SESSION['user_id'], $project['id']);
										$has_log = file_exists($log_file_path);
									?>
									<a href="view_log.php?id=<?php echo $project['id']; ?>" target="_blank" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-secondary text-secondary-foreground hover:bg-secondary/80 h-9 px-3 <?php if (!$has_log) echo 'opacity-50 cursor-not-allowed pointer-events-none'; ?>" title="View Translation Log">Log</a>
